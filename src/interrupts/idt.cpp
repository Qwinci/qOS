...
void registerInterrupt(uint8_t typeAttributes, uint8_t index, uint64_t handler) {
	...
	descriptor->offset0 = reinterpret_cast<uint64_t>(handler) & 0xFFFF;
	descriptor->offset1 = reinterpret_cast<uint64_t>(handler) >> 16 & 0xFFFF;
	descriptor->offset2 = reinterpret_cast<uint64_t>(handler) >> 32 & 0xFFFFFFFF;
